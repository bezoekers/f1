...
with pd.ExcelWriter("data\\xlsx\\data.xlsx") as writer:
    df_circuits.to_excel(writer, sheet_name = "circuits", index = False)
    df_constructor_results.to_excel(writer, sheet_name = "constructor_results", index = False)
    df_constructor_standings.to_excel(writer, sheet_name = "constructor_standings", index = False)
    df_constructors.to_excel(writer, sheet_name = "constructors", index = False)
    df_driver_standings.to_excel(writer, sheet_name = "driver_standings", index = False)
    df_drivers.to_excel(writer, sheet_name = "drivers", index = False)
    df_lap_times.to_excel(writer, sheet_name = "lap_times", index = False)
    df_pit_stops.to_excel(writer, sheet_name = "pit_stops", index = False)
    df_qualifying.to_excel(writer, sheet_name = "qualifying", index = False)
